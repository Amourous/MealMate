...
> - **Backend unit tests** (`TC-B01` to `TC-B11`) — see the [Backend Unit Tests (Jest)](./Testing.md#backend-unit-tests-jest) section.
> - **Frontend integration test** (`TC-F01`) — see the [Frontend Integration Tests (Vitest)](./Testing.md#frontend-integration-tests-vitest) section.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **FR-0** | User Authentication | `AuthContext.jsx`, `backend/routes/auth.js` | JWT-based session management with `bcrypt` password hashing and protected Express middleware. | `TC-B04` – `TC-B09` | Automated (Jest) |
...
| **FR-6** | Pantry Deduction | `PantryManager.jsx`, `GroceryList.jsx` | Logic that subtracts quantities found in `pantry_items` from the total grocery list requirements. | `test-pantry-deduction`, `TC-B02`, `TC-B03` | Manual + Automated |
...
- **Automated Tests**: 11 backend tests (Jest + Supertest) covering auth, pantry CRUD, and API contracts. 1 frontend integration test (Vitest) covering the full user journey (add recipe → grocery list → pantry).
